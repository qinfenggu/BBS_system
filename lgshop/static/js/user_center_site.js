let vm = new Vue({
    ...
    mounted() {
        // 获取省份数据。前端页面刚进来'收货地址'这个页面就执行这个方法
        this.get_provinces();
    },
    watch: {
        // 监听到省份id变化。前端那边前端了省份即form_address.province_id这个值发生变化
    ...
    methods: {
        ...
        // 获取省份数据。这个方法请求url： http://127.0.0.1:8000/areas/视图
        get_provinces(){
            let url = '/areas/';
            axios.get(url, {
                responseType: 'json'
        ...
        // 删除地址。前端那边<span class="del_site" @click="delete_address(index)">×</span>：点击×后就会触发delete_address这个点击事件的方法
        // 前端那边v-for="(address, index) in addresses"这个吗，index是v-for语法里面index的作用。
        // 是一个列表索引值。就是通过这个index可以确定那么多地址中的哪个。
