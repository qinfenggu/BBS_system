...
// 这个就是每次点击图形验证码就会更换一次事件
$(function(){
    //  http://127.0.0.1:5000/captcha/?x=0.2
    $('#captcha-img').click(function (event) {
        var self = $(this);
        // 获取image标签里面的src属性值即{{ url_for('front.graph_captcha') }}即http://127.0.0.1:5000/captcha/
        var src = self.attr('src');
        // param.setParam就是把获取到的src属性值进行处理变成http://127.0.0.1:5000/captcha/?x=随机数
        var newsrc = param.setParam(src,'xx',Math.random());
        // 这个就是把 新的生成src属性值给替换到image标签的src属性里面去
...
//         // md5这个是会进行加密的。发送请求并携带telephone、timestamp、sign参数后，
//        // 会在表单验证处，验证这个sign的值是否和md5(timestamp+telephone+"q3423805gdflvbdfvhsdoa`#$%")加密的值一样。
//        // 不一样就证明你这个请求不是我这个网站请求过来的。主要防止知道我这个发送短信的url后，恶意通过postman这种不断的发送短信
